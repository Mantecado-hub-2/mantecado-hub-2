...
class DatasetDownloadBehavior(TaskSet):
    ...
    def test_download_all_datasets_route(self):
        ...
        with patch.object(DataSetRepository, 'download_all_datasets', return_value=[DataSet(id=1, user_id=1), DataSet(id=2, user_id=1)]):
            response = self.client.get("/dataset/download_all_datasets")

            if response.status_code != 200:
                print(f"Download failed: {response.status_code}")
            elif "Content-Disposition" not in response.headers or "allDatasets.zip" not in response.headers["Content-Disposition"]:
